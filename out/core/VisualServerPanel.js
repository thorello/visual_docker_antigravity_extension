...
class VisualServerPanel {
    static createOrShow(extensionUri, context) {
        ...
        const panel = vscode.window.createWebviewPanel('visualServer', 'Antigravity Dashboard', vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    getSshService(serverId) {
        return this.sshServices.get(serverId);
    }
    async createNativeTerminal(serverId, sshService) {
        const servers = await this.storageService.getServers();
        ...
        try {
            const stream = await sshService.startShell((data) => writeEmitter.fire(data), () => { streamClosed = true; writeEmitter.fire('\r\n[Antigravity Terminal Encerrado]\r\n'); });
            const pty = {
...
exports.VisualServerPanel = VisualServerPanel;
//# sourceMappingURL=VisualServerPanel.js.map